            ...
            if(*move_text != '\0'){
                switch(output_format){
                    ...
                    case SOURCE:
                         ...
                         */
			if(GlobalState.keep_checks) {
			  fprintf(outputfile, " sm %s", (const char *) move_text);
			  something_printed = TRUE;
			}
			else {
			    /* Look for a check or mate symbol. */
				...
				strncpy(just_move, (const char *) move_text, len);
				just_move[len] = '\0';
				fprintf(outputfile, " sm %s", just_move);
				something_printed = TRUE;
				(void) free(just_move);
			    }
			    else {
			      ...
			      something_printed = TRUE;
				   ...
				   break;
                           ...
                           fprintf(outputfile, " sm %s", algebraic);
			   something_printed = TRUE;
                        }
                        break;
                    case LALG:
                    ...
                    Board *final_board)
{
    ...
    unsigned print_move_number = move_number;
